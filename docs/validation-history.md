...
Stage 4c.3 introduced a default-disabled, explicitly armed,
process-lifetime one-shot invocation of
`OnAvailableConcreteModel_ServerInternal`.
...
### Default-disabled runtime acceptance

The unarmed candidate ran on the isolated populated-world clone for a
180-second stability window.
...
Exactly one registration call was made.
...
The server remained stable for 180 seconds after the call. The one-shot
guard prevented additional calls across later scans.
...
The isolated mod and save were restored exactly after both tests. The
normal staged package remained unarmed, and production retained the same
PalServer process and container start time.

### Acceptance boundary

This stage proves that one reflected registration call can complete
without destabilising the isolated populated dedicated server.

It does not yet prove that the target storage retained the foreign chest,
that gameplay behaviour changed, or that repeated calls are idempotent.
Those questions are reserved for Stage 4c.4.
