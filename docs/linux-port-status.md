...
| Stage | Result |
|---|---|
| 4F.1 | Implemented, not yet build/runtime-verified. First cross-camp material **consumption** work (previously only display/transport existed - §4). Root-caused a real-world report ("Insufficient build materials" on a Large Pal Bed despite the checklist showing far more than needed): the checklist total is the guild-wide pool (this mod's transport feature working correctly), but the native build-placement gate (`UPalBuilderComponent::IsExistsMaterialForBuildObject`) only ever sees a camp's REAL local storage, which this mod never populates server-side (client-side minted slots are Windows-client-only and never exist on the dedicated server). Confirmed via user-provided screenshots: the failing camp's real chest held plain Cloth/Wood, not the High Quality Cloth/Hardwood the recipe needed, while the build that *did* succeed at another camp did so because that camp's own real stock/player inventory already had enough, independent of guild pooling. Fix implemented as **periodic, deferred reconciliation**, not a synchronous in-hook transfer: `PalBuilderComponent:OnEnterBaseCamp`/`OnExitBaseCamp` are now also hooked server-side (mirroring the existing client-side hooks) to track which camp each player is currently in; `PalBuilderComponent:IsExistsMaterialForBuildObject` is hooked to read the attempted recipe's four materials and enqueue `{camp, materials}` — matching this file's existing rule that hook callbacks never do reflection/FindAllOf/ProcessEvent work directly, always deferring to the next tick. `process_pending_material_reconciliation()`, run from the same proven-safe tick context as `run_read_only_chest_association`, computes each material's real local shortfall against the guild pool and, if covered, physically moves just the shortfall from a foreign same-guild camp's real slot into an *existing* local slot for that item (writes `StackCount` directly through the same reflected pointers already used for reads elsewhere in this file). If the local camp has no existing slot for that item at all, it logs `RESULT=NO_LOCAL_SLOT` and skips rather than attempting to create one — new-slot creation needs its own safe path and isn't implemented yet. Net effect: a build attempt may fail once and then succeed on a quick retry once the next tick has leveled that camp's stock, rather than working on the very first try. All property offsets consumed at hook-fire time are resolved exactly once via the `PalBuilderComponent` CDO in `resolve_build_gate_layout()` (called from `on_unreal_init`, not from inside any hook), each gated behind an exact property-kind-and-size check before being trusted. Needs a Linux server build + retest against the exact repro (Large Pal Bed at a non-main camp) before shipping. |
